  ...
  peerSend = () => {
    ...
    peer.on('connection', (conn) => {
      ...
      conn.on('open', () => {
        ...
        conn.send({ type: "metadata", value: { name: this.state.file.name, size: this.state.file.size, type: this.state.file.type } })
        conn.send({ type: "buffer", value: this.state.file })
      })
    });
  }
  writeToFile = () => {
    if (!this.state.recvdFile)
      return
    fs.writeFile(downloadPath + "/" + this.state.metadata.name, Buffer.from(this.state.recvdFile), () => {
    ...
    if (this.targetPeer) {
      ...
      return (
        <>
          ...
          {this.state.file ?
            <div className="sharing-link ml-auto mr-auto p-3 rounded m-5">elza://share/?peer={this.state.id}</div>
            : null}
        </>
      )
    }
  ...
  render() {
    return (
      <div className="container p-5 sharing-container">
        ...
        <p className="text-muted"><b>Connection status: </b>{this.state.status} </p>
        {this.chooseMode()}
      </div>
    )
